...
| Pitch Bend / Mod Wheel | Physical keyboard controller inputs | Original OS reads pitch-wheel ADC channel 0 and modulation-wheel ADC channel 2 | Invert conventional channel-1 14-bit Pitch Bend onto ADC channel `0`; invert channel-1 CC1 onto ADC channel `2`; pass Note On/Off directly to the KPC path and ignore MPE expression, Channel Pressure and Polyphonic Key Pressure | Pitch `0..16383`; CC1 `0..127` | Hardware ADC span `0..1023`; OS owns modulation routing and per-voice response | Runtime performance state | Physical EPS wheels are global; the original direct Note-On/Off path is retained for Push compatibility | Original-OS key-on regression proves low/center/high pitch produce ascending ES5505 increments only with the ADC inversion; dense MPE traffic produces only the Note-On/Off KPC packets |
